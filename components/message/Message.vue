    ...
    <div class="flex flex-row items-center justify-between space-x-12 w-full">
      <div class="space-y-2">
        <div class="flex flex-row items-center space-x-4">
          ...
          <h2 class="text-xl font-bold text-gray-900">{{ message.pseudo }}</h2>
        </div>
        <h2 class="text-sm text-gray-500">Dernière mise à jour le {{ formatDate(message.last_updated) }}</h2>
        <button
            @click="toggleModification"
            v-show="(isAdmin || isAuthor(props.message.user_id)) && !isModifying"
            class="text-xl text-indigo-500 font-bold"
        >
          Modifier
        </button>
        <button @click="submitModification" v-show="isModifying" class="text-xl text-indigo-500 font-bold">
          Valider
        </button>
      </div>
